...
class Products
{
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . "/Ecommerce/product_images/" . $uniqueImageName)) {
					...
					if ($q) {
						...
						return ['status' => 303, 'message' => 'Failed to proceed, try again'];
					}
				} else {
	...
	public function deleteProduct($pid = null)
	{
		if ($pid != null) {
			...
			if ($q) {
				...
				return ['status' => 202, 'message' => 'Delete all products in this brand and try again'];
			}
		} else {
			return ['status' => 303, 'message' => 'Invalid product id'];
		}
	...
	public function deleteCategory($cid = null)
	{
		if ($cid != null) {
			...
			if ($q) {
				...
				return ['status' => 202, 'message' => 'Delete all products from this category and try again'];
			}
		} else {
	...
	public function updateDiscount($product_id, $product_discount)
	{
		if ($product_id != null) {
			$q = $this->con->query("UPDATE products SET product_discount = '$product_discount' WHERE product_id = '$product_id'");
			if ($q) {
				return ['status' => 202, 'message' => 'Discount updated'];
			} else {
				return ['status' => 202, 'message' => 'Failed to proceed, try again'];
			}
		} else {
	...
	public function sendEmail($email, $message)
	{
		...
		$phpmailer->Port = 2525;
		$phpmailer->Username = 'd5b1bd421a3935';
		$phpmailer->Password = 'e94b3dd7cda899';

		$phpmailer->setFrom('sales@coirestore.com');
		$phpmailer->addAddress($email);

		$phpmailer->isHTML(true);                                  
		$phpmailer->Subject = 'Order Confirmation';
		$phpmailer->Body    = $message;
		$phpmailer->AltBody = 'order confrimation';
	...
	{
		if ($order_id != null) {
			$q = $this->con->query("UPDATE orders SET p_status = 'Completed' WHERE order_id = '$order_id'");
			if ($q) {
				$q = $this->con->query("SELECT u.user_id, u.email, u.first_name, u.last_name FROM orders o JOIN user_info u ON u.user_id = o.user_id WHERE o.order_id='$order_id'");
				if ($q->num_rows > 0) {
					$row = $q->fetch_assoc();
					$message = "Hello $row[first_name] $row[last_name], Your order #$order_id has been confirmed. Thank you for ordering.";
					$this->sendEmail($row['email'], $message);
					return ['status' => 202, 'message' => 'Order confirmed, Email sent'];
				}
				return ['status' => 202, 'message' => 'Order confirmed'];
			} else {
				return ['status' => 202, 'message' => 'Failed to proceed, try again'];
			}
		} else {
...
if (isset($_POST['update_discount'])) {
	extract($_POST);
	if (!empty($pid) && !empty($e_product_discount)) {
		$p = new Products();
		echo json_encode($p->updateDiscount($pid, $e_product_discount));
		exit();
	} else {
		echo json_encode(['status' => 303, 'message' => 'Invalid details']);
		exit();
...
if (isset($_POST['CONFIRM_ORDER'])) {
	if (!empty($_POST['o_id'])) {
		$p = new Products();
		echo json_encode($p->confirmOrder($_POST['o_id']));
		exit();
	} else {
		echo json_encode(['status' => 303, 'message' => 'Invalid details']);
		exit();
